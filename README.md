...
Le premier livrable couvre le cycle financier utilisable décrit ci-dessus. Les extensions suivantes ne sont pas présentées comme livrées : comptabilité générale/certification fiscale, stocks et achats complets, émission de devis en brouillon, facturation électronique réglementaire, conversion multidevise, publication sur l’App Store et Google Play, synchronisation hors ligne, traductions EN/AR complètes, moteur de commissions avancé et relances automatiques programmées. Les clients natifs Android/iOS ont été ajoutés après le MVP ; leur [validation et leurs limites](docs/mobile/VALIDATION.md) sont documentées séparément. Les factures échues sont détectées à la lecture ; les notifications internes couvrent les opérations du workflow.
